...
# 202005499_9.asm -- Sudoku Solver (9x9 grid)

# java -jar Mars45.jar sm nc 202005499_9.asm < test.txt
...
main:
	...
	li 	$s0, 0			# int i = 0
convert:
	li 	$t1, 81			# Max i val for the loop 
...
mcont:	
	...
	li $s0, 0 			# int i = 0
print:					# Loop to print the solved grid
	beq 	$s0, 9, exit
	li 	$s1,0
print1: 
	beq 	$s1, 9, cprint
...
checker:
	...
	#####preamble######
csquare:	
	li 	$t0, 3			# Max i value for the loop (and inner loop)	
	li 	$s0, 0			# int i = 0
...
csquare1:	
	# Check square
	beq 	$s0, 3, crow		# Exit for loop when i == 3
	li	$s1, 0			# int j = 0
csquare2: # Inner loop
	beq	$s1, 3, ccsquare1
	
	add	$t6, $s0, $t2		# i + (r - r%3) : row
	add	$t7, $s1, $t3		# j + (c - c%3) : column	
...
ccol1:	
	...
	mul	$t2, $s0, 9
	add	$t2, $t2, $a1		# 9*i + c : index of Grid[i][c] 
...
csolver1:		
	li	$s2, 1			# $s2 : int x = 1
solverloop:
	beq 	$s2, 10 , solver_bc0	# Exit loop when x>S
ifvalid:
	move 	$s0, $a0		# copy r
	move	$s1, $a1		# copy c
	move	$a2, $s2		# $a2 : x
